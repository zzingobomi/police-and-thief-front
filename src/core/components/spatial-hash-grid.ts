...
export class SpatialHashGrid extends Component {
  ...
  private getCellIndex(position: any) {
    const x = math.sat(
  ...
  public NewClient(position: any, dimensions: any) {
    const client = {
      position: position,
  ...
  public FindNear(position: any, bounds: any) {
    const [x, y] = position;
    const [w, h] = bounds;
