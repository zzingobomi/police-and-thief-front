  ...
  implements IterableIterator<T>
{
  ...
  remove(...elementsOrIndices: (T | number)[]): boolean {
    const removed = elementsOrIndices.filter((element) =>
      this.removeSingle(element)
    );
    if (removed.length <= 0) return false;
    const removedElements = removed.map((o) =>
      typeof o === "number" ? this.elements[o] : o
    );
    this.updatedFrozenObjects();
   ...
   */
  forEach<U>(
    callbackFn: (element: T, index: number, array: readonly T[]) => void,
    thisArg?: U
  ): void {
   ...
   */
  find<U>(
    predicate: (element: T, index: number, array: readonly T[]) => unknown,
    thisArg?: U
  ): T | undefined {
   ...
   */
  map<U, V>(
    callbackFn: (element: T, index: number, array: readonly T[]) => V,
    thisArg?: U
  ): V[] {
   ...
   */
  every<U>(
    callbackFn: (element: T, index: number, array: readonly T[]) => unknown,
    thisArg?: U
  ): boolean {
