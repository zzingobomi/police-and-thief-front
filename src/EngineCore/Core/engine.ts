...
> extends Dispatcher<EngineListener> {
  ...
  constructor() {
        ...
        onAdded: (...systems: SyncedSystem[]) => {
          ...
          systems.forEach((system) => {
            ...
            const systemListener: SystemListener = {
              ...
              onError: (error) =>
                this.dispatch("onErrorBySystem", error, system),
            };
    ...
    this._entities.addListener(
      {
        onAdded: (...entities: T[]) =>
          this.dispatch("onAddedEntities", ...entities),
        onRemoved: (...entities: T[]) =>
          this.dispatch("onRemovedEntities", ...entities),
  ...
  /**
   * init all entity, component, system.
   */
  init(): void | Promise<void> {
    for (let i = 0; i < this._activeSystems.length; i++)
      this._activeSystems[i].init();
  }
  ...
  protected async runParallel<T>(options?: T): Promise<void> {
    const mapped = this._activeSystems.map((system) =>
      system.run(options, SystemMode.ASYNC)
    );
