 ...
 */
export class Aspect<
  L extends AspectListener = AspectListener
> extends Dispatcher<L> {
  ...
  protected constructor(
    public source: EntityCollection,
    all?: CompType[],
    exclude?: CompType[],
    one?: CompType[]
  ) {
    ...
    this.listener = {
      onAdded: (...entities: SyncedEntity[]) => {
        ...
        this.updateFrozen();
        (<Dispatcher<AspectListener>>this).dispatch(
          "onAddedEntities",
          ...added
        );
      },
      ...
      onSorted: () => {
        if (this.filteredEntities.length === 0) return;
        this.filteredEntities = this.source.filter(
          this.matches,
          this
        ) as SyncedEntity[];
        this.updateFrozen();
  ...
  matches(entity: AbstractEntity): boolean {
    ...
    if (
      ...
      this.excludeComponents.some(testFn)
    )
      return false;
  ...
  static for(
    ...
    const entities =
      collOrEngine instanceof Engine ? collOrEngine.entities : collOrEngine;
